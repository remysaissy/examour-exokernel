...
void		_8259a_init(void)
{
  ...
  /* ICW1: init. */
  outbp(PIC_MASTER_CMD, ICW1_INIT | ICW1_ICW4);

  /* ICW2 */
  /* irqs in idt from IDT_BASE_IRQ to IDT_BASE_IRQ+15 */
  outbp(PIC_MASTER_IMR, IRQ_BASE_IDX);

  /* ICW3 */
  /* slave is on IR2 */
  outbp(PIC_MASTER_IMR, (1UL << PIC_CASCADE_IR));

  /* ICW4 */
  outbp(PIC_MASTER_IMR, ICW4_8086);

  /* PIC SLAVE */

  /* ICW1 */
  outbp(PIC_SLAVE_CMD, ICW1_INIT | ICW1_ICW4);

  /* intr in idt from 0x28 to 0x2f */
  outbp(PIC_SLAVE_IMR, IRQ_BASE_IDX+8);

  /* ICW3 */
  /* PIC is slave on master's IR2.. */
  outbp(PIC_SLAVE_IMR, PIC_CASCADE_IR);

  outbp(PIC_SLAVE_IMR, ICW4_8086);
