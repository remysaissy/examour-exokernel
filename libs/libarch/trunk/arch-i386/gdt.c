...
bool    gdt_create(gdt_t *context)
{
  if (context == NULL)
    return (false);
  memset(context, 0, GDT_TABLE_SIZE * sizeof(*context));
  return (true);
}

/* NOTE: We assume that the context passed is not the current gdt context. */
...
{
  if (context == NULL || limit == 0 || index == 0)
    return (false);
